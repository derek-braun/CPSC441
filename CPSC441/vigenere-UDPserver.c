...
int main()
{
        ...
        for( ; ; )
        {
                ...
                char cipherKey[MAX_BUFFER_SIZE];
                sprintf(cipherKey, "%s", "derek");

                for(int i = 0; i < strlen(messagein); i++){
                  if(messagein[i] >= 97 && messagein[i] <= 122){
                    if(messagein[i] + 13 > 122){
                      messagein[i] -= 13;
                    }
                    else{
                      messagein[i] += 13;
                  ...
                  else if(messagein[i] >= 65 && messagein[i] <= 90){
                    if(messagein[i] + 13 > 90){
                      messagein[i] -= 13;
                    }
                    else{
                      messagein[i] += 13;
